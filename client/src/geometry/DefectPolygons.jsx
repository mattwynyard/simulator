...
const DefectPolygon = (props) => {
    ...
    props.data.geojson.forEach(point => {
        const xy = map.latLngToContainerPoint(point);
        const vector = new Vector2D(xy.x, xy.y)
        vectors.push(vector);
    });
    let xyVectors = null;
    if (vectors.length === 2) {
        ...
        xyVectors = buildComplexPolygon(vectors, thickness, props.id)
    }
    if (xyVectors !== null) {
        xyVectors.forEach((vector) => {
            try {
                const latlng = map.containerPointToLatLng(vector.vector)
                latlngs.push(latlng)
            } catch (err) {
                //console.log(xyVectors)
            }
        });
    }
    if (latlngs) {
